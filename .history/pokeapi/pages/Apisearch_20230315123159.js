...
<main className="min-h-screen flex flex-col justify-center items-center py-4 bg-fixed bg-cover bg-center h-screen" style={{ backgroundImage:"url('pikachu running.gif')" }}>
  ...
  {!choose ? (
    <h1 className="text-center text-4xl font-bold mb-4 text-gray-800">Enter your Pokemon!!</h1>
  ) : (
    <div className="flex justify-between items-center">
      <div className="flex flex-col justify-center items-center mr-10">
        <h1 className="text-4xl font-bold mb-2 text-yellow-500">{PokeEl.name}</h1>
        <img src={PokeEl.img} alt={PokeEl.name} className="w-64 h-64 mb-4"/>
        <h3 className="text-lg mb-2 font-bold text-gray-800">Species: {PokeEl.species}</h3>
        <h3 className="text-lg mb-2 font-bold text-gray-800">Type: {PokeEl.type}</h3>
        <h4 className="text-base mb-2 font-bold text-gray-800">Hp: {PokeEl.hp}</h4>
        <h4 className="text-base mb-2 font-bold text-gray-800">Attack: {PokeEl.attack}</h4>
        <h4 className="text-base mb-2 font-bold text-gray-800">Defense: {PokeEl.defense}</h4>
      </div>
      <div className="flex flex-col justify-center items-center">
        <h1 className="text-4xl font-bold mb-2 text-yellow-500">{PokeIL.name}</h1>
        <img src={PokeIL.images} alt={PokeIL.name} className="w-64 h-64 mb-4"/>
      </div>
    </div>
  )}
</div>
